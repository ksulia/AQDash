...
export class RealTime extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        // const now = new Date(this.props.state.completeTime.split("T")[0])
        // // const utc = new Date(now.getTime() + now.getTimezoneOffset() * 60000);
        // const sunset = getSunset(42.6526, -73.7562, now);
        // const utc = new Date(sunset.getTime() + sunset.getTimezoneOffset() * 60000);

        const currDate = new Date(this.props.state.completeTime.split("T")[0])
        const getTimes = SunCalc.getTimes((currDate.setDate(currDate.getDate() + 1)), 42.6526, -73.7562);
        const sunrise = getTimes.sunrise.toISOString(), sunset = getTimes.sunset.toISOString();
        if (this.props.state.completeTime != prevProps.state.completeTime) {
            console.log('HERE', this.props.state.completeTime, prevProps.state.completeTime, sunrise, sunset)
            this.handleChange({ daytime: (this.props.state.completeTime > sunrise && this.props.state.completeTime < sunset) })
        }
